function barcodeData = recognizeBarcode(img)
    ...
    for i=1:4:size(code,2)
        ...
        if ~isempty(map(1,ismember(map(2,:),str)))
            barcodeData = strcat(barcodeData,map(1,ismember(map(2,:),str)));
            par = strcat(par,"O");
        elseif ~isempty(map(1,ismember(map(3,:),str)))
            barcodeData = strcat(barcodeData,map(1,ismember(map(3,:),str)));
            par = strcat(par,"E");
        elseif ~isempty(map(1,ismember(map(4,:),str)))
            barcodeData = strcat(barcodeData,map(1,ismember(map(4,:),str)));
            par = strcat(par,"E");
        else
            if code(1,i) == 0
                idx = knnsearch(left,code(2,[i,i+1,i+2,i+3]));
                barcodeData = strcat(barcodeData,int2str(left_m(idx)));
                par = strcat(par,left_p(idx));
            else
                idx = knnsearch(right,code(2,[i,i+1,i+2,i+3]));
                barcodeData = strcat(barcodeData,int2str(right_m(idx)));
                par = strcat(par,'E');
            end
        end
    end

    barcodeData = strcat(map(1,ismember(map(5,:),par)),barcodeData);
    disp(barcodeData)
end